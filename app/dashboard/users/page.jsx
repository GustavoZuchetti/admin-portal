...
// ── Estilos base ─────────────────────────────────────────────────────
const inp = { width:'100%', background:'var(--surface3)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, color:'var(--text1)', padding:'10px 13px', fontSize:13, outline:'none', fontFamily:'inherit' }
...
const lbl = { fontSize:11, color:'var(--text2)', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.6px', display:'block', marginBottom:6 }
const card = { background:'var(--surface)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:12 }
const btnPrimary = { background:'var(--brand)', color:'#fff', border:'none', borderRadius:8, padding:'10px 18px', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', boxShadow:'0 4px 12px rgba(59,130,246,0.25)' }
const btnGhost  = { background:'transparent', color:'var(--text4)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, padding:'10px 18px', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }
...
function forcaSenha(p) {
  ...
  if (pts <= 1) return { label:'Fraca', color:'var(--danger)', w:'25%' }
  if (pts <= 2) return { label:'Razoável', color:'var(--warning)', w:'50%' }
  if (pts <= 3) return { label:'Boa', color:'var(--brand)', w:'75%' }
  return { label:'Forte', color:'var(--success)', w:'100%' }
}
    ...
    <>
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:999, width:540, maxHeight:'92vh', overflowY:'auto', ...card, padding:28, boxShadow:'0 32px 72px rgba(0,0,0,0.7)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24 }}>
          <div>
            <h2 style={{ fontSize:17, fontWeight:800, color:'var(--text1)', margin:0 }}>Adicionar Usuário</h2>
            <p style={{ fontSize:12, color:'var(--text4)', marginTop:3 }}>Crie uma conta imediata ou envie um link de convite</p>
          </div>
          <button onClick={onClose} style={{ background:'none', border:'none', color:'var(--text4)', fontSize:22, cursor:'pointer', lineHeight:1, padding:'0 4px' }}>×</button>
        </div>
        ...
        {!sucesso && (
          <div style={{ display:'flex', background:'var(--bg)', borderRadius:10, padding:4, marginBottom:22, gap:4 }}>
            {[['criar','Criar conta agora'],['convidar','Convidar por link']].map(([id,label])=>(
              <button key={id} onClick={()=>{setAba(id);setErro('')}} style={{ flex:1, padding:'8px 0', borderRadius:7, border:'none', fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.15s', background: aba===id?'var(--surface2)':'transparent', color: aba===id?'var(--text1)':'var(--text4)', boxShadow: aba===id?'0 1px 4px rgba(0,0,0,0.4)':'' }}>
                {label}
              </button>
            ))}
          </div>
        ...
        {sucesso && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{ background:'rgba(34,197,94,0.08)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:10, padding:'14px 16px', color:'var(--success)', fontSize:13, fontWeight:600 }}>
              {sucesso.tipo==='conta' ? 'Usuário criado com sucesso!' : `${sucesso.links.length} convite(s) gerado(s)`}
            </div>

            {sucesso.tipo === 'conta' ? (
              <>
                <div style={{ background:'var(--surface3)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, padding:'16px 18px', display:'flex', flexDirection:'column', gap:10 }}>
                  <div style={{ fontSize:12, color:'var(--text4)', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.5px' }}>Credenciais para compartilhar</div>
                  {[['E-mail', sucesso.email],['Senha temporária', sucesso.senha],['Organização', sucesso.org]].map(([k,v])=>(
                    <div key={k} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <code style={{ fontSize:13, color:'var(--text1)', fontFamily:'monospace' }}>{v}</code>
                        <button onClick={()=>copiar(v)} style={{ background:'rgba(59,130,246,0.1)', color:'var(--brand-text)', border:'1px solid rgba(59,130,246,0.2)', borderRadius:5, padding:'2px 8px', fontSize:10, cursor:'pointer', fontWeight:700 }}>Copiar</button>
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{ background:'rgba(245,158,11,0.07)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:8, padding:'10px 13px', fontSize:12, color:'var(--warning)', lineHeight:1.6 }}>
                  Compartilhe as credenciais com segurança. Oriente o usuário a trocar a senha no primeiro acesso.
                </div>
              </>
            ) : (
              sucesso.links.map((l,i)=>(
                <div key={i} style={{ background:'var(--surface3)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, padding:'14px 16px' }}>
                  <div style={{ fontSize:11, fontWeight:700, color:'var(--text2)', marginBottom:8 }}>{l.orgNome}</div>
                  <div style={{ fontSize:11, color:'var(--brand-text)', wordBreak:'break-all', marginBottom:8, lineHeight:1.5 }}>{l.link}</div>
                  <button onClick={()=>copiar(l.link)} style={{ background:'rgba(59,130,246,0.1)', color:'var(--brand-text)', border:'1px solid rgba(59,130,246,0.2)', borderRadius:6, padding:'5px 12px', fontSize:11, cursor:'pointer', fontWeight:700 }}>Copiar link</button>
                </div>
              ))
            )}
        ...
        {!sucesso && aba === 'criar' && (
          <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <div style={{ background:'rgba(59,130,246,0.05)', border:'1px solid rgba(59,130,246,0.15)', borderRadius:8, padding:'10px 13px', fontSize:12, color:'var(--brand-text)', lineHeight:1.6 }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                <div style={{ position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', display:'flex', gap:6, alignItems:'center' }}>
                  <button type="button" onClick={()=>setVerSenha(v=>!v)}
                    style={{ background:'transparent', border:'none', color:'var(--text2)', cursor:'pointer', padding:'3px 6px', fontSize:11.5, fontWeight:600, textDecoration:'underline', textUnderlineOffset:2 }}>
            ...
            {erro && <div style={{ background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:8, padding:'10px 13px', color:'var(--danger)', fontSize:13 }}>{erro}</div>}
        ...
        {!sucesso && aba === 'convidar' && (
          <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <div style={{ background:'rgba(139,92,246,0.05)', border:'1px solid rgba(139,92,246,0.15)', borderRadius:8, padding:'10px 13px', fontSize:12, color:'var(--purple)', lineHeight:1.6 }}>
            ...
            <div>
              ...
              <p style={{ fontSize:11, color:'var(--text4)', marginBottom:10 }}>Um link separado é gerado por organização.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {orgs.map(org => {
                  const sel_ = orgsSelec.includes(org.id)
                  return (
                    <label key={org.id} onClick={()=>toggleOrg(org.id)} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 14px', background:sel_?'rgba(59,130,246,0.08)':'var(--surface3)', border:`1px solid ${sel_?'rgba(59,130,246,0.35)':'rgba(255,255,255,0.07)'}`, borderRadius:8, cursor:'pointer', transition:'all 0.15s' }}>
                      <div style={{ width:18, height:18, borderRadius:5, background:sel_?'var(--brand)':'transparent', border:`2px solid ${sel_?'var(--brand)':'var(--text4)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all 0.15s' }}>
                        {sel_ && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
                      </div>
                      <div>
                        <div style={{ fontSize:13, fontWeight:600, color:sel_?'var(--brand-text)':'var(--text1)' }}>{org.nome}</div>
                        <div style={{ fontSize:11, color:'var(--text4)' }}>{org.plano} · {org.status}</div>
                      </div>
                    </label>
                  )
                })}
            ...
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={onClose} style={{ ...btnGhost, flex:1 }}>Cancelar</button>
              <button onClick={gerarConvites} disabled={loading} style={{ ...btnPrimary, flex:2, background:'var(--purple)', boxShadow:'var(--shadow-md)', opacity:loading?0.7:1 }}>
...
export default function Users() {
  ...
  const roleColor = r => r==='super_admin'?'var(--danger)':r==='org_admin'?'var(--brand)':'var(--text4)'
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24 }}>
        <div>
          ...
          <p style={{ color:'var(--text2)', fontSize:13, marginTop:4 }}>Gerencie acessos por organização</p>
        </div>
      ...
      <div style={{ ...card, overflow:'hidden' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
          ...
          <tbody>
            {loading
              ? <tr><td colSpan={4} style={{ padding:40, textAlign:'center', color:'var(--text4)' }}>Carregando...</td></tr>
              : filtrados.length === 0
              ? <tr><td colSpan={4} style={{ padding:40, textAlign:'center', color:'var(--text4)' }}>Nenhum usuário encontrado.</td></tr>
              : filtrados.map((u,i)=>(
                <tr key={u.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.05)', background:i%2===0?'transparent':'rgba(255,255,255,0.01)' }}>
                  <td style={{ padding:'12px 16px', color:'var(--text1)', fontWeight:500 }}>{u.email||'—'}</td>
                  <td style={{ padding:'12px 16px' }}>
                    {u.organizations
                      ? <span style={{ display:'inline-flex', alignItems:'center', gap:6 }}>
                          <span style={{ color:'var(--text2)' }}>{u.organizations.nome}</span>
                          <span style={{ fontSize:10, fontWeight:700, color:u.organizations.status==='Ativo'?'var(--success)':'var(--warning)', background:u.organizations.status==='Ativo'?'rgba(34,197,94,0.1)':'rgba(245,158,11,0.1)', padding:'1px 6px', borderRadius:20 }}>{u.organizations.status}</span>
                        </span>
                  ...
                  <td style={{ padding:'12px 16px', color:'var(--text4)', fontSize:12 }}>{new Date(u.created_at).toLocaleDateString('pt-BR')}</td>
                </tr>
              ))}
          </tbody>
        </table>
